...
RETURN:
- `x` : the new optimised parameter
- `f`  : a table of all function values:
     `f[1]` is the value of the loss function before any optimization and
     `f[#f]` is the final fully optimized value of loss, at `x*`]]
    optim.adagrad(feval, x, state) --x是param fed as feval的input
